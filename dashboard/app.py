...
def get_cached_info(symbol: str):
    ...
    url = (
        f"https://api.polygon.io/v3/reference/tickers/{symbol}?apiKey={POLYGON_API_KEY}"
    )
    res = requests.get(url).json()
    if "results" not in res:
...
def api_search(query: str):
    try:
        res = yf_session.get(
            f"https://query2.finance.yahoo.com/v1/finance/search?q={query}&quotesCount=6&newsCount=0"
        )
...
def api_tickers():
    ...
    for symbol in STOCK_SYMBOLS:
        ...
        if latest:
            ...
            results.append(
                {"symbol": symbol, "close": None, "change": 0, "change_pct": 0}
            )
...
def api_candles(symbol: str):
    ...
    return jsonify(
        {"symbol": symbol.upper(), "candles": candles, "count": len(candles)}
    )
...
def api_history(symbol: str):
    ...
    if not candles:
        ...
        row = query_one(
            "SELECT close FROM stock_ohlcv_1m WHERE symbol=%s ORDER BY bucket_start DESC LIMIT 1;",
            (symbol.upper(),),
        )
        if row:
            ...
            tick_row = query_one(
                "SELECT price FROM raw_stock_ticks WHERE symbol=%s ORDER BY timestamp DESC LIMIT 1;",
                (symbol.upper(),),
            )
...
def api_summary_fundamentals(symbol: str):
    try:
        ...
        logger.warning(
            f"Failed to fetch fundamentals for {symbol}: {e}. Using mock data fallback."
        )
        return jsonify(
            {
                "symbol": symbol,
...
def api_financials(symbol: str):
    try:
        ...
        logger.warning(
            f"Failed to fetch financials for {symbol}: {e}. Using mock data fallback."
        )
        return jsonify(
            {
                "2026": {
...
def api_earnings(symbol: str):
    ...
    try:
        ...
        for date_str, row in df.iterrows():
            if math.isnan(row.get("EPS Estimate", float("nan"))) and math.isnan(
                row.get("Reported EPS", float("nan"))
            ):
                continue
            ...
            history.append(
                {
                    ...
                    "estimate": (
                        None
                        if math.isnan(row.get("EPS Estimate", float("nan")))
                        else float(row.get("EPS Estimate"))
                    ),
                    "reported": (
                        None
                        if math.isnan(row.get("Reported EPS", float("nan")))
                        else float(row.get("Reported EPS"))
                    ),
                    "surprise": (
                        None
                        if math.isnan(row.get("Surprise(%)", float("nan")))
                        else float(row.get("Surprise(%)"))
                    ),
                }
            )
        ...
        logger.warning(
            f"Failed to fetch earnings for {symbol}: {e}. Using mock data fallback."
        )
        return jsonify(
            {
                "upcoming": {
...
def api_analysis(symbol: str):
    ...
    def generate_is_row(base, growth_range, format_str="{:.3f}B"):
        ...
        for i in range(8):
            g = (
                random.uniform(-growth_range, growth_range + 0.05)
                if i > 0
            ...
            "rows": [
                {
                    ...
                    "subtitle": "Growth YoY",
                    "info": True,
                    "values": generate_is_row(416.168, 0.1, "{:.3f}B"),
                },
                {
                    "name": "Cost of Goods Sold",
                    "subtitle": "Growth YoY",
                    "info": True,
                    "values": generate_is_row(220.968, 0.05, "{:.3f}B"),
                },
                {
                    "name": "Gross Profit",
                    "subtitle": "Growth YoY",
                    "info": False,
                    "values": generate_is_row(195.208, 0.08, "{:.3f}B"),
                },
                {
                    "name": "Operating Expenses",
                    "subtitle": "Growth YoY",
                    "info": False,
                    "values": generate_is_row(62.158, 0.1, "{:.3f}B"),
                },
                {
                    "name": "Operating Income",
                    "subtitle": "Growth YoY",
                    "info": False,
                    "values": generate_is_row(133.058, 0.1, "{:.3f}B"),
                },
                {
                    "name": "Other Income (Expense)",
                    "subtitle": "Growth YoY",
                    "info": False,
                    "values": generate_is_row(-321.0, 1.5, "{:.0f}M"),
                },
                {
                    "name": "Pre Tax Income",
                    "subtitle": "Growth YoY",
                    "info": False,
                    "values": generate_is_row(132.738, 0.1, "{:.3f}B"),
                },
                {
                    "name": "Tax Provision",
                    "subtitle": "Growth YoY",
                    "info": False,
                    "values": generate_is_row(21.218, 0.2, "{:.3f}B"),
                },
                {
                    "name": "Net Profit",
                    "subtitle": "Growth YoY",
                    "info": False,
                    "values": generate_is_row(112.018, 0.1, "{:.3f}B"),
                },
                {
                    "name": "Diluted EPS",
                    "subtitle": "Growth YoY",
                    "info": True,
                    "values": generate_is_row(7.43, 0.1, "{:.2f}"),
                },
                {
                    "name": "Diluted Average Shares",
                    "subtitle": "",
                    "info": False,
                    "values": generate_is_row(15.008, 0.02, "{:.3f}B"),
                },
                {
                    "name": "Normalized EBITDA",
                    "subtitle": "",
                    "info": True,
                    "values": generate_is_row(144.758, 0.1, "{:.3f}B"),
                },
                {
                ...
                },
                {
                    ...
                    "subtitle": "",
                    "info": True,
                    "values": generate_is_row(46.91, 0.02, "{:.2f}%"),
                },
                {
                    "name": "Operating Expenses %",
                    "subtitle": "",
                    "info": True,
                    "values": generate_is_row(14.93, 0.02, "{:.2f}%"),
                },
                {
                    "name": "Operating Income %",
                    "subtitle": "",
                    "info": True,
                    "values": generate_is_row(31.97, 0.02, "{:.2f}%"),
                },
                {
                    "name": "Net Profit %",
                    "subtitle": "",
                    "info": True,
                    "values": generate_is_row(26.80, 0.02, "{:.2f}%"),
                },
            ],
        },
        "categories": [
                ...
                "metrics": [
                    {
                        ...
                        "avg5": 6.77,
                        "ind": 23.03,
                        "data": generate_series(5.0, 0.5, 0.2),
                        "ind_data": generate_series(20.0, 1.0, 0.5),
                    },
                    {
                        "name": "Earnings",
                        "current": 2.01,
                        "avg5": 1.81,
                        "ind": 2.33,
                        "data": generate_series(1.5, 0.2, 0.05),
                        "ind_data": generate_series(2.0, 0.2, 0.05),
                        "is_main": True,
                        "subtitle": "Stock prices follow Earnings Per Share, which are trending downward.",
                    },
                    {
                        "name": "Free Cash Flow",
                        "current": 1.82,
                        "avg5": 1.81,
                        "ind": 2.18,
                        "data": generate_series(1.5, 0.2),
                        "ind_data": generate_series(2.0, 0.2),
                    },
                    {
                        "name": "Dividend",
                        "current": 0.26,
                        "avg5": 0.25,
                        "ind": 0.29,
                        "data": generate_series(0.22, 0.01, 0.01),
                        "ind_data": generate_series(0.25, 0.01, 0.01),
                    },
                    {
                        "name": "Book Value",
                        "current": 7.23,
                        "avg5": 4.73,
                        "ind": 39.58,
                        "data": generate_series(5.0, 0.5, 0.3),
                        "ind_data": generate_series(35.0, 1.0, 0.5),
                    },
                ],
            },
            {
                ...
                "metrics": [
                    {
                        ...
                        "avg5": "5.45%",
                        "ind": "0.37%",
                        "data": generate_series(10.0, 2.0),
                        "ind_data": generate_series(2.0, 1.0),
                        "is_main": True,
                        "subtitle": "Revenues are growing faster than the PC Devices average.",
                    },
                    {
                        "name": "EPS YoY",
                        "current": "22.04%",
                        "avg5": "11.63%",
                        "ind": "35.22%",
                        "data": generate_series(15.0, 3.0),
                        "ind_data": generate_series(30.0, 5.0),
                    },
                    {
                        "name": "FCF YoY",
                        "current": "30.89%",
                        "avg5": "14.75%",
                        "ind": "94.66%",
                        "data": generate_series(20.0, 4.0),
                        "ind_data": generate_series(80.0, 10.0),
                    },
                    {
                        "name": "Dividends YoY",
                        "current": "3.98%",
                        "avg5": "4.04%",
                        "ind": "4.62%",
                        "data": generate_series(4.0, 0.2),
                        "ind_data": generate_series(4.5, 0.2),
                    },
                    {
                        "name": "BV YoY",
                        "current": "63.00%",
                        "avg5": "17.87%",
                        "ind": "30.04%",
                        "data": generate_series(40.0, 5.0),
                        "ind_data": generate_series(25.0, 3.0),
                    },
                ],
            },
            {
                ...
                "metrics": [
                    {
                        ...
                        "avg5": "46.64%",
                        "ind": "44.70%",
                        "data": generate_series(45.0, 1.0),
                        "ind_data": generate_series(44.0, 1.0),
                    },
                    {
                        ...
                        "avg5": "31.52%",
                        "ind": "24.33%",
                        "data": generate_series(30.0, 1.0),
                        "ind_data": generate_series(24.0, 1.0),
                        "is_main": True,
                        "subtitle": "Operating margin is high relative to the PC Devices average.",
                    },
                    {
                        "name": "Net Margin",
                        "current": "26.60%",
                        "avg5": "26.50%",
                        "ind": "20.95%",
                        "data": generate_series(25.0, 1.0),
                        "ind_data": generate_series(20.0, 1.0),
                    },
                    {
                        ...
                        "avg5": "150.49%",
                        "ind": "0.06%",
                        "data": generate_series(130.0, 10.0),
                        "ind_data": generate_series(5.0, 2.0),
                    },
                    {
                ...
                "metrics": [
                    {
                        ...
                        "avg5": "8.28x",
                        "ind": "11.08x",
                        "data": generate_series(9.0, 0.5),
                        "ind_data": generate_series(10.0, 0.5),
                    },
                    {
                        ...
                        "avg5": "31.62x",
                        "ind": "39.78x",
                        "data": generate_series(35.0, 2.0),
                        "ind_data": generate_series(38.0, 2.0),
                        "is_main": True,
                        "subtitle": "AAPL is a good value based on its PE Ratio compared to the PC Devices average.",
                    },
                    {
                    ...
                    },
                    {
                        ...
                        "avg5": "47.27x",
                        "ind": "24.89x",
                        "data": generate_series(45.0, 2.0),
                        "ind_data": generate_series(25.0, 1.0),
                    },
                    {
                        "name": "EV/EBITDA",
                        "current": "-",
                ...
                "metrics": [
                    {
                        ...
                        "avg5": "297.95%",
                        "ind": "335.89%",
                        "data": generate_series(250.0, 20.0),
                        "ind_data": generate_series(320.0, 20.0),
                        "is_main": True,
                        "subtitle": "Debt to EBITDA is below the PC Devices average.",
                    },
                    {
                        ...
                        "avg5": "145.70%",
                        "ind": "-61.26%",
                        "data": generate_series(100.0, 10.0),
                        "ind_data": generate_series(-50.0, 10.0),
                    },
                    {
                        ...
                        "ind": "-0.44x",
                        "data": generate_series(4.0, 0.2),
                        "ind_data": generate_series(-0.5, 0.2),
                    },
                    {
                        "name": "Quick Ratio",
                        "current": "1.02x",
                        "avg5": "0.91x",
                        "ind": "1.03x",
                        "data": generate_series(0.95, 0.05),
                        "ind_data": generate_series(1.0, 0.05),
                    },
                    {
                        "name": "Current Ratio",
                        "current": "1.07x",
                        "avg5": "0.95x",
                        "ind": "1.17x",
                        "data": generate_series(1.0, 0.05),
                        "ind_data": generate_series(1.1, 0.05),
                    },
                ],
            },
            {
                ...
                "metrics": [
                    {
                        ...
                        "avg5": "0.29x",
                        "ind": "0.30x",
                        "data": generate_series(0.29, 0.01),
                        "ind_data": generate_series(0.3, 0.01),
                    },
                    {
                        ...
                        "avg5": "8.53x",
                        "ind": "1.66x",
                        "data": generate_series(8.5, 0.2),
                        "ind_data": generate_series(1.7, 0.1),
                    },
                    {
                        ...
                        "avg5": "3.71x",
                        "ind": "3.04x",
                        "data": generate_series(3.5, 0.1),
                        "ind_data": generate_series(3.0, 0.1),
                    },
                    {
                        ...
                        "ind": "5.04%",
                        "data": generate_series(9.0, 0.5),
                        "ind_data": generate_series(5.0, 0.5),
                        "is_main": True,
                        "subtitle": "Return on assets is higher than the PC Devices average.",
                    },
                    {
                        ...
                        "avg5": "14.60%",
                        "ind": "14.54%",
                        "data": generate_series(13.5, 0.5),
                        "ind_data": generate_series(14.5, 0.5),
                    },
                ],
            },
        ],
...
def api_company_info(symbol: str):
    try:
        ...
        logger.warning(
            f"Failed to fetch company info for {symbol}: {e}. Using mock data fallback."
        )
        ...
        mock = {
            ...
            "GOOGL": {
                ...
                "holders": [
                    ...
                    {
                        "holder": "State Street Investment Management (US)",
                        "pct": 0.0336,
                    },
                    {"holder": "Geode Capital Management, L.L.C.", "pct": 0.0202},
                    {
                        "holder": "Fidelity Management & Research Company LLC",
                        "pct": 0.0169,
                    },
                ],
                "top_buyers": [
...
def api_health():
            ...
            services.append(
                {
                    "service": "kafka",
                    "status": "ok",
                    "message": f"{kafka_host}:{kafka_port}",
                }
            )
    except Exception:
        services.append(
            {"service": "kafka", "status": "pending", "message": "Unreachable"}
        )
    ...
    try:
        ...
        if row and row.get("last_update"):
            ...
            if age < 90:
                services.append(
                    {"service": "producer", "status": "ok", "message": "Ticks flowing"}
                )
            else:
                services.append(
                    {
